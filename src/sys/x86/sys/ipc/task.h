...
{
    PeripheralRegionAddr = 0x40000000,
...
requires(nv::common::is_power_of_2(stack_size) && stack_size >= configMINIMAL_STACK_SIZE)
// coverity[definition]
// coverity[entity_not_emitted]
struct TaskStack
{
    alignas(stack_size) std::array<StackType_t, stack_size> _stack;
