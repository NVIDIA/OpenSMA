...
get_security_version(const ParsingFwType InputParseingFwType)
{
    ...
    ImageHeader image_header{};
    auto        image_header_span_view = to_span_view(image_header);
    int         read_complete_size = read_from_flash(image_header_span_view, ImageBaseAddress);
    if (read_complete_size != sizeof(decltype(image_header))) {
        return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FlashAccessFail);
    }
    // get container header address
    uint32_t container_header_address = image_header.offset_to_extended_header
                                      + ImageBaseAddress;

    // get container header
    ContainerHeader container_header{};
    auto            container_header_span_view = to_span_view(container_header);
    read_complete_size = read_from_flash(container_header_span_view, container_header_address);
    if (read_complete_size != sizeof(decltype(container_header))) {
        return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FlashAccessFail);
    }

    // check if the container header is valid
    if (!ContainerHeader::validate_container_header(container_header)) {
        return std::unexpected<ParsingErrorCode>(
            ParsingErrorCode::ContainerHeaderMagicValueNotMatch);
    }
...
get_image_signing_key_version(const ParsingFwType InputParseingFwType)
{
    ...
    const Address ContainerHeaderAddress = image_header.offset_to_extended_header
                                         + ImageBaseAddress;

    // get container header
    ContainerHeader container_header{};
    auto            container_header_span_view = to_span_view(container_header);
    uint32_t        read_complete_size         = read_from_flash(container_header_span_view,
                                                  ContainerHeaderAddress);
    if (read_complete_size != sizeof(decltype(container_header))) {
        return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FlashAccessFail);
    }
    if (!ContainerHeader::validate_container_header(container_header)) {
    ...
    const uint32_t       SignatureBlockOffset  = container_header.sig_blk_offset;
    const Address        SignatureBlockAddress = SignatureBlockOffset + ContainerHeaderAddress;
    SignatureBlockHeader signature_block{};
    auto                 signature_block_span_view = to_span_view(signature_block);
    read_complete_size = read_from_flash(signature_block_span_view, SignatureBlockAddress);
    if (read_complete_size != sizeof(decltype(signature_block))) {
        return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FlashAccessFail);
    }
    if (!SignatureBlockHeader::validate_signature_block_header(signature_block)) {
        return std::unexpected<ParsingErrorCode>(
            ParsingErrorCode::SignatureBlockHeaderMagicValueNotMatch);
    }
    if (signature_block.certificate_offset == 0) {
        if (InputParseingFwType == ParsingFwType::Fmc) {
            // if the fw is fmc, return 0xff, since the isk certificate is not used in fmc
            return 0xff;
        }
        return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FwNotUseIskCertificate);
    }
    CertificateBlockHeader isk_certificate_header{};
    auto isk_certificate_header_span_view = to_span_view(isk_certificate_header);
    read_complete_size                    = read_from_flash(isk_certificate_header_span_view,
                                         SignatureBlockAddress
                                             + signature_block.certificate_offset);
    if (read_complete_size != sizeof(decltype(isk_certificate_header))) {
        return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FlashAccessFail);
    }
    if (!CertificateBlockHeader::validate_certificate_block_header(isk_certificate_header)) {
        return std::unexpected<ParsingErrorCode>(
            ParsingErrorCode::CertificateBlockHeaderMagicValueNotMatch);
    }

    return isk_certificate_header.fuse_version;
}
...
get_fw_metadata_hash_range(const ParsingFwType InputParseingFwType)
{
    ...
    // second hash
    {
        const Address ImageBaseAddress = get_fw_image_address(InputParseingFwType);
        ImageHeader   image_header{};
        auto          image_header_span_view = to_span_view(image_header);
        int read_complete_size = read_from_flash(image_header_span_view, ImageBaseAddress);
        if (read_complete_size != sizeof(decltype(image_header))) {
        ...
        ContainerHeader container_header{};
        auto            container_header_span_view = to_span_view(container_header);
        read_complete_size                         = read_from_flash(container_header_span_view,
                                             ContainerHeaderAddress);
        if (read_complete_size != sizeof(decltype(container_header))) {
            return std::unexpected<ParsingErrorCode>(ParsingErrorCode::FlashAccessFail);
        }
        ...
        metadata_hash_range[1] = HashRangeItem{ContainerHeaderAddress, container_header.length};
    }
