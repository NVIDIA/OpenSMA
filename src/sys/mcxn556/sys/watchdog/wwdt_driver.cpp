...
void LogWdtFault(uint32_t msp_frame, uint32_t lr_value)
{
    ...
    const uint32_t          ctimer_tick = nv::ctimer::Driver::read_ticks();
    memcpy(fault_buffer.data(), &ctimer_tick, sizeof(ctimer_tick));
    memcpy(fault_buffer.data() + sizeof(ctimer_tick), &lr_value, sizeof(lr_value));
    ...
    if (lr_value == nv::common::to_underlying(nv::logger::ExcReturn::HandlerModeMSP)
        ...
        memcpy(fault_buffer.data() + sizeof(lr_value) + sizeof(ctimer_tick),
               MSP + StackOffset,
               CopySize);
    }
        ...
        memcpy(fault_buffer.data() + sizeof(lr_value) + sizeof(ctimer_tick), PSP, CopySize);
    }
    ...
    uint32_t last_switch_tick = perfmon_driver.last_tick;
    memcpy(fault_buffer.data() + sizeof(bit) + sizeof(PSP) + sizeof(MSP),
           &last_switch_tick,
           sizeof(last_switch_tick));
