                                  ...
                                  uint32_t&         rx_channel)
{
    switch (flexcomm) {
        ...
        case nv::spi::Flexcomm::Seven:
            ...
            break;
        case nv::spi::Flexcomm::Eight:
            tx_channel = kDma0RequestMuxLpFlexcomm8Tx;
            rx_channel = kDma0RequestMuxLpFlexcomm8Rx;
            break;
        case nv::spi::Flexcomm::Nine:
            tx_channel = kDma0RequestMuxLpFlexcomm9Tx;
            rx_channel = kDma0RequestMuxLpFlexcomm9Rx;
            break;
        default:
            tx_channel = 0;
            rx_channel = 0;
...
void EdmaDriver::init()
{
    ...
    const uint32_t dma_rx_channel = static_cast<uint32_t>(_flexcomm);  // Even channel for RX
    const uint32_t dma_tx_channel = static_cast<uint32_t>(_flexcomm) + 1;  // Odd channel for TX
