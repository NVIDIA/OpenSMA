...
NV_PRIVILEGED_FUNCTION status_t Fccob_Driver_Phrase_Write_Priv(nv::flash::Address       address,
                                                               ...
                                                               uint32_t                 offset)
{
    return FccobDriver::phrase_write_impl(address, buffer, offset);
}
...
status_t FccobDriver::phrase_write(nv::flash::Address       address,
                                   ...
                                   uint32_t                 offset)
{
    return phrase_write_svc(address, buffer, offset);
}
                                                   ...
                                                   uint32_t                 offset)
{
    ...
    __asm volatile(  // NOLINT
        ...
        : "memory");
#else
    return phrase_write_impl(address, buffer, offset);
#endif
    // coverity[cert_msc52_cpp_violation] - Expect no return
}

NV_PRIVILEGED_FUNCTION status_t FccobDriver::phrase_write_impl(nv::flash::Address       address,
                                                               const nv::flash::Buffer& buffer,
                                                               uint32_t                 offset)
{
    volatile FMUTEST_Type* reg   = FMU0TEST;
    ...
    for (uint32_t i = 0; i < 4; i++) {
        *std::bit_cast<volatile uint32_t*>(address + i * 4) = *std::bit_cast<uint32_t*>(
            static_cast<const uint8_t*>(buffer.data()) + i * 4 + offset);
    }
                                           ...
                                           const std::span<uint8_t>& buffer)
{
    ...
    __asm volatile(  // NOLINT
        ...
        : "memory");
#else
    return read_impl(address, length, buffer);
#endif
    // coverity[cert_msc52_cpp_violation] - Expect no return
}
