...
{
    // Only create task if it can be accessed directly on the current core
    assert(sys::ipc::task::Driver::can_direct_access_on_current_core(id()) == true);
    ...
    handle() = xTaskCreateStatic(
        entrypoint,
        _name.data(),
        static_cast<uint16_t>(stack_region.size() / sizeof(StackType_t)),
        this,
        static_cast<UBaseType_t>(priority),
        reinterpret_cast<StackType_t*>(stack_region.data()),
        &tcb());
...
{
    // coverity[cert_arr36_c_violation] - valid
