...
, Info(QueueInfos.at(common::to_underlying(id)))
{
    ...
    // Only create queue if it is defined in this core
    if (sys::ipc::task::Driver::is_on_same_core(id)) {
        ...
        for (auto buf = super.static_queue_buffer.begin();
            ...
            // Only add offset if the queue is defined in this core
            if (sys::ipc::task::Driver::is_on_same_core(core)) {
                buf += len * item_size;
            }
        }
...
Queue::Status Queue::send(const ConstItem& item, Usecs timeout)
{
    // pre-conditions
