...
Event::Event(EventId id) : sys::ipc::Event(), _id(id)
{
    ...
    // Only create event if it is defined in this core
    if (sys::ipc::task::Driver::is_on_same_core(_id)) {
...
Event::Return Event::bits(bool from_isr) const
{
    if (sys::ipc::task::Driver::is_on_same_core(_id)) {
...
Event::Status Event::set(Bits bits, bool from_isr)
{
    if (sys::ipc::task::Driver::is_on_same_core(_id)) {
        if (from_isr) {
...
Event::Status Event::clear(Bits bits, bool from_isr)
{
    if (sys::ipc::task::Driver::is_on_same_core(_id)) {
        if (from_isr) {
