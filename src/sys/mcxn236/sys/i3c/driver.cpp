...
, _is_gpu(is_gpu)
{
    constexpr uint32_t Clock = 25000000UL;
    /// create configuration
    i3c_master_config_t master_config;
    I3C_MasterGetDefaultConfig(&master_config);
    master_config.baudRate_Hz.i2cBaud          = freq.i2c;
    master_config.baudRate_Hz.i3cPushPullBaud  = freq.i3c_pp;
    master_config.baudRate_Hz.i3cOpenDrainBaud = freq.i2c_od;
    master_config.enableOpenDrainStop          = false;
    master_config.disableTimeout               = true;

    // CX8 uses 50:50 duty cycle
    master_config.enableOpenDrainHigh = is_gpu ? true : false;
    I3C_Type* base                    = nullptr;
    ...
    // init controller
    I3C_MasterInit(base, &master_config, Clock);
    ...
    // coverity[cert_exp60_cpp_violation] Wave this until I have a good solution
    I3C_MasterTransferCreateHandleEDMA(
        base, &_i3c_m_handle, &Callback, this, &_rx_edma_handle, &_tx_edma_handle);
