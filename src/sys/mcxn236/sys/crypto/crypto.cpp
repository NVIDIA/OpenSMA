                                         ...
                                         nboot_img_auth_ecdsa_parms_t& parms)
{
    ...
    __asm volatile(  // NOLINT
        ...
        : "memory");
#else
    return Crypto_Image_Auth_Ecdsa_Priv(nbootCtx, imageAddress, is_signature_verified, parms);
#endif
                                                     ...
                                                     nboot_img_auth_ecdsa_parms_t& parms)
{
    ...
    using namespace nv::fw_parser;
    constexpr std::array<size_t, 3> ValidAddress = {
        FmcFwAddress, ActiveFwAddress, InactiveFwAddress};

    // check if the image address is valid
    if (imageAddress == nullptr
        or std::find(
               ValidAddress.begin(), ValidAddress.end(), std::bit_cast<size_t>(imageAddress))
               == ValidAddress.end()) {
...
authenticate_firmware(const nv::fw_parser::ParsingFwType InputParseingFwType)
{
    using namespace nv::spdm::crypto;
    ...
    if (nv::flash::Status::Ok != nv::flash::Flash::read_secure_fw_version(secure_fw_version)
        || nv::flash::Status::Ok != nv::flash::Flash::read_key_revoke(image_key_revoke)) {
        return CryptoStatus::FailCfpaAccess;
    }

    // check security fw version between fw and device
    auto svn_on_firmware = get_security_version(InputParseingFwType);
    if (!svn_on_firmware) {
    ...
    auto image_key_version_on_firmware = get_image_signing_key_version(InputParseingFwType);
    ...
    auto imageAddress = std::bit_cast<uint8_t*>(get_fw_image_address(InputParseingFwType));
