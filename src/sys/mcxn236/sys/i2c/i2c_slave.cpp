...
// #include "sys/mcxn236/sys/i2c/i2c_slave.h"
...
, address_valid_callback(driver_config.address_valid_callback)
, _task_state(Init)
{
...
void I2CSlaveDriver::service_tx_request(lpi2c_slave_transfer_t* transfer)
{
    if (_task_state == Nacking) {
        ...
        _process_data_callback(address,
                               true,
                               _tx_buffer,
                               _tx_buffer_transfer_size,
                               _parent_task_class,
...
void I2CSlaveDriver::send_ack_or_nack(lpi2c_slave_transfer_t* transfer)
{
    ...
    switch (_task_state) {
        ...
        case Idle: {
            ...
            // NOLINTNEXTLINE: nxp api name
            uint8_t    address      = transfer->receivedAddress >> 1U;  // remove r/w bit
            const bool ValidAddress = address_valid_callback(address, _parent_task_class);
