                                 ...
                                 void*                                   user_data)
{
    ...
    switch (completion_status) {
        case kStatus_Success  : task->set_event(Task::Event::CtrlDone, true); break;
        case kStatus_LPI2C_Nak: task->set_event(Task::Event::CtrlNak, true); break;
        case kStatus_LPI2C_ArbitrationLost:
            task->set_event(Task::Event::CtrlArbLost, true);
            break;
        case kStatus_Lpi2c_StateWar:
            nv::logger::Logger::add_from_isr(
                nv::logger::Event::I2CStateInvalid.unique_id, nv::logger::Level::Error, {});
            task->set_event(Task::Event::CtrlError, true);
            break;
        default:
    ...
    switch (transfer->event) {
        ...
        case kLPI2C_SlaveCompletionEvent:
            ...
            if (!context->transmit) {
                ...
                context->buffer[0] = static_cast<uint8_t>(
                    std::min<uint32_t>(transfer->transferredCount + 1U, ByteMask));
                context->buffer[1] = static_cast<uint8_t>(base->SAMR & ByteMask) | 0U;
...
{
    constexpr uint8_t ByteMask = 0xFFU;
    return static_cast<uint8_t>(_base->SAMR & ByteMask) >> 1U;
}

bool Driver::send_i2c_recovery()
{
    constexpr uint8_t TargetAddress = 0x30;
    constexpr uint8_t TargetOffset  = 0x00;

    if (!_base) {
        return false;
    }

    // First write: 0x00 to address 0x30 offset 0x00
    ...
    // Write offset and 0x00 data in single transaction
    std::array<uint8_t, 2> write_data_0 = {TargetOffset, 0x00};
    xfer.data                           = write_data_0.data();
    xfer.dataSize                       = write_data_0.size();

    status_t status = LPI2C_MasterTransferBlocking(_base, &xfer);
    if (status != kStatus_Success) {
        return false;
    }

    // 10ms delay between the two writes
    vTaskDelay(pdMS_TO_TICKS(10));

    // Second write: 0x01 to address 0x30 offset 0x00
    // Write offset and 0x01 data in single transaction
    std::array<uint8_t, 2> write_data_1 = {TargetOffset, 0x01};
    xfer.data                           = write_data_1.data();
    xfer.dataSize                       = write_data_1.size();

    status = LPI2C_MasterTransferBlocking(_base, &xfer);
    if (status != kStatus_Success) {
        return false;
    }
