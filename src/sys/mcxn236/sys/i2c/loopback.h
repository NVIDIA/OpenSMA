...
// MCXN236 does not currently support a real I2C controller/target loopback mode
// like MCXN556. This header provides the same API so higher layers (selftest,
// I2C task) can build, and the implementation will report "NotI2C"/skipped.
...
public:
    ...
    // Keep the same pattern/shape as MCXN556 for compatibility.
    static constexpr size_t TestTimes = 100;

    lpi2c_slave_handle_t _handle{};

    LPI2C_Type*   _base{nullptr};
    nv::i2c::Port _port{};
