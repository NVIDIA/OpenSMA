...
void Task::make()
{
    constexpr auto StackSize = std::max(1984, int(configMINIMAL_STACK_SIZE));
...
bool Task::is_background_copy_enabled()
{
    ...
    nv::logger::info(nv::logger::Event::BackgroundCopyInitSetup,
                     {static_cast<uint8_t>(background_copy_policy),
                      static_cast<uint8_t>(background_copy_policy_one_time)});
...
[[noreturn]] void Task::main()
{
    ...
    auto wait_bits = static_cast<std::underlying_type_t<PldmEventBits>>(
    ...
    while (true) {
        ...
        if (event & Task::PldmEventBits::EventRequestBit) {
            ...
            if (queue_status == Queue::Status::Ok) {
                switch (request.type) {
                    case RequestType::AuthFinish: {
                        uint8_t verify_cc = 0;
                        get_authentication_result(request.buffer[0], verify_cc);
...
nv::pldm::Status Task::pldm_tx(const nv::mctp::Packet& packet)
{
    ...
    event_status = (packet.priv.packet_length > NV_PLDM_MCTP_HDR_PAYLOAD_SIZE)
                     ? event.set(Task::PldmEventBits::PldmTaskRx4kBit, false)
                     : event.set(Task::PldmEventBits::PldmTaskRxBit, false);
...
pldm::Status Task::pldm_bg_start()
{
    ...
    auto        event_status = event.set(Task::PldmEventBits::BgStartBit, false);
...
{
    ipc::Event& event        = Event::make(EventId::PldmTask);
    auto        event_status = event.set(Task::PldmEventBits::BgInitBit, false);
...
void Task::on_timer([[maybe_unused]] ipc::Timer& id)
{
    ...
    static_cast<void>(event.set(Task::PldmEventBits::TimerBit, false));
}
