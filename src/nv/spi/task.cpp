...
void Task::start()
{
    ...
    while (true) {
        ...
        if (_queue.size() != 0) {
            (void)_event.set(EventBits::TxEvent, false);
        }
    }
}
...
Task::Status Task::tx(const mctp::Packet& packet)
{
    ...
    // Set TxEvent
    auto event_status = Event::make(event_id).set(EventBits::TxEvent, false);
...
void Task::forward(Buffer& buffer)
{
    ...
    mctp::Status status{};
    if (is_routed == false) {
        if (_client == mctp::Client::Spi0) {
            status = mctp::Driver::mctp_send_from_spi_0(item);
        }
        else if (_client == mctp::Client::Spi1) {
            status = mctp::Driver::mctp_send_from_spi_1(item);
        }
        else if (_client == mctp::Client::Spi2) {
            status = mctp::Driver::mctp_send_from_spi_2(item);
        }
        else {
            return;
        }
...
Task::Status Task::update_routing_table(mctp::Client client)
{
    ...
    // Set TxEvent
    auto event_status = Event::make(event_id).set(EventBits::RoutingTableEvent, false);
    if (event_status != Event::Status::Ok) {
...
{
    auto status = nv::ipc::Event::make(event_id).set(nv::spi::EventBits::MbxEvent, true);
