...
I2cStatus Emc1812::get_remote_high_temp(int8_t& temp_integer)
{
    uint8_t temp_high_byte = 0;
...
I2cStatus Emc1812::set_remote_high_alert_threshold(int8_t threshold)
{
    // Write signed threshold directly
    // coverity[cert_int31_c_violation]
    return write_reg(Register::RemoteTempHighLimit, static_cast<uint8_t>(threshold));
}

I2cStatus Emc1812::get_remote_high_alert_threshold(int8_t& threshold)
{
    uint8_t threshold_byte = 0;
    auto    status         = read_reg(Register::RemoteTempHighLimit, threshold_byte);
    if (status == I2cStatus::Ok) {
        // Convert back to signed int8_t
        // coverity[cert_int31_c_violation]
        threshold = static_cast<int8_t>(threshold_byte);
...
I2cStatus Emc1812::set_local_thermal_limit(int8_t threshold)
{
    // Write signed threshold directly
    // coverity[cert_int31_c_violation]
    return write_reg(Register::LocalThermalLimit, static_cast<uint8_t>(threshold));
}

I2cStatus Emc1812::get_local_thermal_limit(int8_t& threshold)
{
    uint8_t threshold_byte = 0;
    auto    status         = read_reg(Register::LocalThermalLimit, threshold_byte);
    if (status == I2cStatus::Ok) {
        // Convert back to signed int8_t
        // coverity[cert_int31_c_violation]
        threshold = static_cast<int8_t>(threshold_byte);
...
I2cStatus Emc1812::set_ext1_thermal_limit(int8_t threshold)
{
    // Write signed threshold directly
    // coverity[cert_int31_c_violation]
    return write_reg(Register::Ext1ThermalLimit, static_cast<uint8_t>(threshold));
}

I2cStatus Emc1812::get_ext1_thermal_limit(int8_t& threshold)
{
    uint8_t threshold_byte = 0;
    auto    status         = read_reg(Register::Ext1ThermalLimit, threshold_byte);
    if (status == I2cStatus::Ok) {
        // Convert back to signed int8_t
        // coverity[cert_int31_c_violation]
        threshold = static_cast<int8_t>(threshold_byte);
