...
Status Task::request_from_isr(const Item& request)
{
    ...
    // Event from ISR
    auto event      = ipc::Event::make(ipc::EventId::LogEvent);
    auto set_status = event.set(EventBits::AddLogISREvent, true);
...
{
    const nv::logger::Entry LogEntry{.timestamp = request.timestamp,
                                     .level     = request.level,
                                     .event     = request.event,
                                     .data      = request.data};
...
{
    std::array<uint8_t, DumpCheckSize + AsciiStrLen> dump_buffer_raw{};
    dump_buffer_raw[0]     = DumpHeadMagicRaw;
    dump_buffer_raw.back() = DumpTailMagicRaw;
    std::memcpy(
        dump_buffer_raw.data() + 1, ascii_arr.ascii_arr.data(), ascii_arr.ascii_arr.size());
...
bool Task::log_debug_token()
{
    using namespace nv::ipc;
    EventData  data{};
    const auto isTokenInstalled = debugtoken::is_dbg_token_in_flash() == true ? 1 : 0;
