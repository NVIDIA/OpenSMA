...
TEST_F(Pca9555Test, I2cReadOperations) {
    ...
    std::array<uint8_t, sys::i2c::I2CSlaveDriver::BufferSize> write_data = {0x02, 0x55};  // Write to output register
...
TEST_F(Pca9555Test, I2cWriteOperations) {
    std::array<uint8_t, sys::i2c::I2CSlaveDriver::BufferSize> write_data;
...
TEST_F(Pca9555Test, InputPinUpdates) {
    ...
    std::array<uint8_t, sys::i2c::I2CSlaveDriver::BufferSize> write_data = {0x06, 0xFF, 0xFF};
    fixture.pca.i2c_write(write_data, 3);
...
TEST_F(Pca9555Test, RequiredMasks) {
    ...
    emulation::Pca9555 pca_masked(0xFFFF, 0x00FF, 0xFF00);  // First 8 pins required as outputs, last 8 as inputs
    
    uint16_t direction_vector;
    pca_masked.get_pin_directions(direction_vector);
    ensure::is_eq(direction_vector, 0xFF00);  // First 8 pins should be outputs (0), last 8 inputs (1)
    
    // Try to change direction of required pins
    std::array<uint8_t, sys::i2c::I2CSlaveDriver::BufferSize> write_data = {0x06, 0xFF, 0x00};
