...
public:
    ...
    {
        const uint8_t fatalFault = EIPayloadList::FatalFaultEI;
...
TEST_F(nsmtest, get_supported_nvidia_message_types)
{
    ...
    if constexpr (nv::ipc::Enable_Nsm_type5) {
        unsigned int bitmask  = (1u << static_cast<unsigned int>(
                                    NsmMsgType::DeviceConfiguration));
        res[7]               |= static_cast<uint8_t>(bitmask);
    }
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x01, 0x05, 0xA0, 0x00, 0x02, NvPldmMcuComponentIdPrefix, 0x00};  // 0A 00 ->
                                                                                // A0 00
    //          nvmsg| cmd | code| reserved  | data size
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x01, 0x05, 0x0A, 0x00, NvPldmMcuComponentIdPrefix, 0x02, 0x00};  // 02 FF ->
                                                                                // FF 02
    //          nvmsg| cmd | code| reserved  | data size
    arr8_res res{0x06, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00};
    ensure::is_eq(fixture.sendRecv_nsm(req, res, 8), true);
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x01, 0x05, 0x0A, 0x00, 0x02, NvPldmMcuComponentIdPrefix, 0x01};  // 00 -> 01
    //          nvmsg| cmd | code| reserved  | data size
    arr8_res res{0x06, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00};
    ensure::is_eq(fixture.sendRecv_nsm(req, res, 8), true);
};
...
TEST_F(nsmtest, get_rot_info_invalid_len)
{
    ...
    arr8_req req{0x06, 0x01, 0x05, 0x0A, 0x00, 0x02, NvPldmMcuComponentIdPrefix};  // missing
                                                                          // comp_class_idx
    //          nvmsg| cmd | code| reserved  | data size
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x03, 0x05, 0xA0, 0x00, 0x02, NvPldmMcuComponentIdPrefix, 0x00};  // 0A 00 ->
                                                                                // A0 00
    //          nvmsg| cmd | code| reserved  | data size
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x03, 0x05, 0x0A, 0x00, NvPldmMcuComponentIdPrefix, 0x02, 0x00};  // 02 FF ->
                                                                                // FF 02
    //          nvmsg| cmd | code| reserved  | data size
    arr8_res res{0x06, 0x03, 0x02, 0x00, 0x00, 0x00, 0x00};
    ensure::is_eq(fixture.sendRecv_nsm(req, res, 8), true);
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x03, 0x05, 0x0A, 0x00, 0x02, NvPldmMcuComponentIdPrefix, 0x01};  // 00 -> 01
    //          nvmsg| cmd | code| reserved  | data size
    arr8_res res{0x06, 0x03, 0x02, 0x00, 0x00, 0x00, 0x00};
    ensure::is_eq(fixture.sendRecv_nsm(req, res, 8), true);
};
...
{
    //          nvmsg| cmd | size| comp_class| comp_id
    arr8_req req{0x06, 0x03, 0x05, 0x0A, 0x00, 0x02, NvPldmMcuComponentIdPrefix};  // missing idx
    //          nvmsg| cmd | code| reserved  | data size
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x05, 0x05, 0xA0, 0x00, 0x02, NvPldmMcuComponentIdPrefix, 0x00};  // 0A 00 ->
                                                                                // A0 00
    //          nvmsg| cmd | code| reserved  | data size
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x05, 0x05, 0x0A, 0x00, NvPldmMcuComponentIdPrefix, 0x02, 0x00};  // 02 FF ->
                                                                                // FF 02
    //          nvmsg| cmd | code| reserved  | data size
    arr8_res res{0x06, 0x05, 0x02, 0x00, 0x00, 0x00, 0x00};
    ensure::is_eq(fixture.sendRecv_nsm(req, res, 8), true);
...
{
    //          nvmsg| cmd | size| comp_class| comp_id                | comp_class_idx
    arr8_req req{0x06, 0x05, 0x05, 0x0A, 0x00, 0x02, NvPldmMcuComponentIdPrefix, 0x01};  // 00 -> 01
    //          nvmsg| cmd | code| reserved  | data size
    arr8_res res{0x06, 0x05, 0x02, 0x00, 0x00, 0x00, 0x00};
    ensure::is_eq(fixture.sendRecv_nsm(req, res, 8), true);
};
...
{
    //          nvmsg| cmd | size| comp_class| comp_id
    arr8_req req{0x06, 0x05, 0x05, 0x0A, 0x00, 0x02, NvPldmMcuComponentIdPrefix};  // missing idx
    //          nvmsg| cmd | code| reserved  | data size
