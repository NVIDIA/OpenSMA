...
[[noreturn]] void Task::main()
{
    ...
    auto init_status = _driver.init(mctp_rx_buffer.data(),
                                    mctp_rx_buffer.data(),
                                    hid_rx_buffer.data(),
                                    hid_rx_buffer.data());
    if (init_status != 0) {
    ...
    while (true) {
        ...
        if (active_bits & MctpRxBit) {
            _event.clear(MctpRxBit, false);
            mctp_receive();
        }

        /* HidRxBit */
        if (active_bits & HidRxBit) {
            _event.clear(HidRxBit, false);
            hid_receive();
        }

        /* MctpTxDoneBit */
        if (active_bits & MctpTxDoneBit) {
            _event.clear(MctpTxDoneBit, false);
            // enable tx
...
void Task::mctp_receive()
{
    ...
    while (offset < BufferSize) {
        ...
        if (usb_pkt.usb_hdr.dmtf_id != UsbDmtfId
            ...
            if (is_routed == false) {
                auto status = mctp::Driver::mctp_send_from_us_usb(item);
...
usb::Status Task::usb_tx(ipc::Queue::Item& item)
{
    ...
    auto event_status = event.set(MctpTxBit, false);
...
usb::Status Task::set_mctp_rx0_event()
{
    ...
    auto& event        = Event::make(EventId::UsbTask);
    auto  event_status = event.set(MctpRxBit, true);
...
{
    auto& event        = Event::make(EventId::UsbTask);
    auto  event_status = event.set(HidRxBit, true);
...
{
    auto& event        = Event::make(EventId::UsbTask);
    auto  event_status = event.set(MctpTxDoneBit, true);
...
{
    auto& event        = Event::make(EventId::UsbTask);
    auto  event_status = event.set(AttachBit, true);
...
{
    auto& event        = Event::make(EventId::UsbTask);
    auto  event_status = event.set(ResetBit, true);
...
{
    auto& event        = Event::make(EventId::UsbTask);
    auto  event_status = event.set(UpdateRoutingTableBit, false);
