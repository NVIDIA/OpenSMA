...
void test_event(bool isr)
{
    ...
    for (auto i = EventId::Test1; i < EventId::Test4; i++) {
        ...
        ensure::is_eq(ev.bits(isr), None);
        ensure::is_eq(ev.set(Pattern, isr), Status::Ok);
        if (isr) {
            ev.wait(Pattern, false, true, 1ms);
        }
        ensure::is_eq(ev.bits(isr), Pattern);
        ensure::is_eq(ev.clear(Mask, isr), Status::Ok);
        if (isr) {
            ev.wait(Pattern, false, true, 1ms);
        }
        ensure::is_eq(ev.bits(isr), Result);
        ensure::is_eq(ev.clear(All, isr), Status::Ok);
        if (isr) {
            ev.wait(Pattern, false, true, 1ms);
        }
        ensure::is_eq(ev.bits(isr), None);
        ...
        for (uint32_t b = 0; b < nbits; b++) {
            ev.set(common::bit(b), isr);
            if (isr) {
                ev.wait(Pattern, false, true, 1ms);
            }
            val |= common::bit(b);
            ensure::is_eq(ev.bits(isr).value(), val);
        }
        for (uint32_t b = 0; b < nbits; b++) {
            ev.clear(common::bit(b), isr);
            if (isr) {
                ev.wait(Pattern, false, true, 1ms);
            }
            val &= common::mask(b);
