...
public:
    ...
    static bool is_address_valid_callback(uint8_t& address, void* this_task_instance);
...
private:
    // Helper Functions
    // ---------------------------------------------
    // to tell the i2c driver if the current address is valid to ACK/NACK
    bool is_valid_address(const uint8_t& address);
