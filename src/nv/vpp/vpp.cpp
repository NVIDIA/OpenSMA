...
, i2c_driver_config({config.i2c_bus, i2c_callback, is_address_valid_callback, this})
...
bool VPP::is_address_valid_callback(uint8_t& address, void* this_task_instance)
{
    // NOLINTNEXTLINE: no way around this without function pointers
    return static_cast<VPP*>(this_task_instance)->is_valid_address(address);
}
bool VPP::is_valid_address(const uint8_t& address)
{
    uint8_t drive_index = 0;
    if (i2c_address_to_gpio_index(address, drive_index)) {
        return true;
    }
    // invalid address
    return false;
