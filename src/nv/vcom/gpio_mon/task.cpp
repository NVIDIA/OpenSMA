...
#include "nv/vcom/gpio_mon/task.h"
...
// Restore the interrupt configuration to the project's original configuration.
// Otherwise, the interrupt will be disabled.
void disable_or_restore(uint8_t bank, uint8_t pin)
{
    for (const auto& [port, cfg_pin, det, sel] : nv::ipc::GpioInterruptSetup) {
        if (port == bank && cfg_pin == pin) {
            (void)nv::gpio::Driver::init_interrupt(bank, pin, det, sel);
            return;
        }
    }
    (void)nv::gpio::Driver::init_interrupt(
        bank, pin, nv::gpio::InterruptDetection::InterruptDisabled, MonitorChannel);
}
