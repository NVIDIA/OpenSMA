...
{
    AutoInstallExitHook() { std::atexit(nv::ut::Coverage::on_complete); }
} nv_ut_coverage_on_exit;
...
void Coverage::on_complete()
{
    ...
    if (!details::Reporter::inst.cfg().quiet) {
        nv::info("Dumping coverage data\n");
    }
    ...
    while (info_start != info_end) {
        ...
        switch (info->version) {
            case 0x4233322a:
            case 0x42333220:  // allow override
                break;
            // GBS:BEGIN NO COVERAGE FIXME!!
            default:
                nv::fatal("compiler version not supported\n");
                // GBS:END NO COVERAGE FIXME!!
        }

        uint32_t ver = 0x4233322a;  // B32*
