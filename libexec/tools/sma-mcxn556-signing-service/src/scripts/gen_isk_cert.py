...
class IskCertGenerator:
    ...
    def load_template(self) -> dict:
        ...
        """
        template = os.path.join(self.workspace, 'config', 'isk_cert.yaml')
        with open(template, 'r') as stream:
            yaml = YAML()
            data = yaml.load(stream)
        return data

    def get_isk_pubkey_location(self, key_num: int) -> str:
    ...
    def get_isk_sign_provider(self) -> str:
        ...
        return (
            f'type=nvsp;key_number={self.rotk};key_label={self.key_label};'
    ...
    def generate(self) -> None:
        ...
        config['public_key_0'] = self.get_isk_pubkey_location(self.isk)
        config['signer_0'] = self.get_isk_sign_provider()
        config['fuse_version'] = 1 << (self.isk - 1) if self.isk > 0 else 0

        # Dump ISK certificate yaml config file
        cfgfile = self.tmppath('isk_cert.yaml')
...
def main(argv):
    ...
    generator = IskCertGenerator(args.env, args.key_dir, args.key_label, args.rotk, args.outfile, args.isk)
    generator.generate()
