...
package body RFLX.SPDM_Responder.Opaque_Data with
  SPARK_Mode
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM_Responder.Opaque_Data.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM_Responder.Opaque_Data.Valid_Predecessor (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Opaque_Data.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.SPDM_Responder.Opaque_Data.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM_Responder.Opaque_Data.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Opaque_Data.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM_Responder.Opaque_Data.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Responder.Opaque_Data.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM_Responder.Opaque_Data.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.SPDM_Responder.Opaque_Data.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Opaque_Data.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Responder.Opaque_Data.Valid_Size (Ctx, Fld, RFLX.SPDM_Responder.Opaque_Data.Field_Size (Ctx, Fld))
       and then RFLX.SPDM_Responder.Opaque_Data.Available_Space (Ctx, Fld) >= RFLX.SPDM_Responder.Opaque_Data.Field_Size (Ctx, Fld)
       and then RFLX.SPDM_Responder.Opaque_Data.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM_Responder.Opaque_Data.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Initialize_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Opaque_Data.Field_First (Ctx, RFLX.SPDM_Responder.Opaque_Data.F_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
