...
package body RFLX.SPDM_Emu.Platform_Port with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, [F_Command => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, [F_Command => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM_Emu.Platform_Port.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM_Emu.Platform_Port.Valid_Predecessor (Ctx, Fld);
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          when F_Command =>
             Invalid (Ctx.Cursors (F_Transport_Type)),
          when F_Transport_Type =>
             Invalid (Ctx.Cursors (F_Size)),
          when F_Size =>
             Invalid (Ctx.Cursors (F_SPDM_Payload)),
          when F_SPDM_Payload =>
             True));
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM_Emu.Platform_Port.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.SPDM_Emu.Platform_Port.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_SPDM_Payload then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   function Get_SPDM_Payload (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_SPDM_Payload).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_SPDM_Payload).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_SPDM_Payload;

   procedure Get_SPDM_Payload (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_SPDM_Payload).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_SPDM_Payload).Last);
   begin
   ...
   procedure Generic_Get_SPDM_Payload (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_SPDM_Payload).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_SPDM_Payload).Last);
   begin
      Process_SPDM_Payload (Ctx.Buffer.all (First .. Last));
   end Generic_Get_SPDM_Payload;
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM_Emu.Platform_Port.Has_Buffer (Ctx)
       and then RFLX.SPDM_Emu.Platform_Port.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM_Emu.Platform_Port.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Emu.Platform_Port.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM_Emu.Platform_Port.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.SPDM_Emu.Platform_Port.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM_Emu.Platform_Port.Valid_Next (Ctx, Fld)
       and then Fld in F_Command | F_Transport_Type | F_Size
       and then RFLX.SPDM_Emu.Platform_Port.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Emu.Platform_Port.Valid_Size (Ctx, Fld, RFLX.SPDM_Emu.Platform_Port.Field_Size (Ctx, Fld))
       and then RFLX.SPDM_Emu.Platform_Port.Available_Space (Ctx, Fld) >= RFLX.SPDM_Emu.Platform_Port.Field_Size (Ctx, Fld)
       and then RFLX.SPDM_Emu.Platform_Port.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM_Emu.Platform_Port.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Command (Ctx : in out Context; Val : RFLX.SPDM_Emu.Command) is
   begin
      Set_Scalar (Ctx, F_Command, RFLX.SPDM_Emu.To_Base_Integer (Val));
   end Set_Command;

   procedure Set_Transport_Type (Ctx : in out Context; Val : RFLX.SPDM_Emu.Transport_Type) is
   begin
      Set_Scalar (Ctx, F_Transport_Type, RFLX.SPDM_Emu.To_Base_Integer (Val));
   end Set_Transport_Type;

   procedure Set_Size (Ctx : in out Context; Val : RFLX.SPDM_Emu.Size) is
   begin
      Set_Scalar (Ctx, F_Size, RFLX.SPDM_Emu.To_Base_Integer (Val));
   end Set_Size;

   procedure Set_SPDM_Payload_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_SPDM_Payload, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_SPDM_Payload_Empty;

   procedure Initialize_SPDM_Payload_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Emu.Platform_Port.Has_Buffer (Ctx)
       ...
       and then RFLX.SPDM_Emu.Platform_Port.Field_First (Ctx, RFLX.SPDM_Emu.Platform_Port.F_SPDM_Payload) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
       and Structural_Valid (Ctx, F_SPDM_Payload)
       and Field_Size (Ctx, F_SPDM_Payload) = RFLX_Types.To_Bit_Length (Length)
       and Ctx.Verified_Last = Field_Last (Ctx, F_SPDM_Payload)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_SPDM_Payload);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_SPDM_Payload) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_SPDM_Payload);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_SPDM_Payload) := (State => S_Structural_Valid, First => First, Last => Last, Value => 0, Predecessor => Ctx.Cursors (F_SPDM_Payload).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_SPDM_Payload)) := (State => S_Invalid, Predecessor => F_SPDM_Payload);
   end Initialize_SPDM_Payload_Private;

   procedure Initialize_SPDM_Payload (Ctx : in out Context) is
   begin
      Initialize_SPDM_Payload_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_SPDM_Payload)));
   end Initialize_SPDM_Payload;

   procedure Set_SPDM_Payload (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_SPDM_Payload));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_SPDM_Payload_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_SPDM_Payload)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_SPDM_Payload)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_SPDM_Payload))) = Data);
   end Set_SPDM_Payload;

   procedure Generic_Set_SPDM_Payload (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_SPDM_Payload);
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (First);
      Buffer_Last : constant RFLX_Types.Index := RFLX_Types.To_Index (First + RFLX_Types.To_Bit_Length (Length) - 1);
   begin
      Process_SPDM_Payload (Ctx.Buffer.all (Buffer_First .. Buffer_Last));
      Initialize_SPDM_Payload_Private (Ctx, Length);
   end Generic_Set_SPDM_Payload;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Command := Get_Command (Ctx);
      Struct.Transport_Type := Get_Transport_Type (Ctx);
      Struct.Size := Get_Size (Ctx);
      Struct.SPDM_Payload := [others => 0];
      Get_SPDM_Payload (Ctx, Struct.SPDM_Payload (Struct.SPDM_Payload'First .. Struct.SPDM_Payload'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_SPDM_Payload)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Command (Ctx, Struct.Command);
      Set_Transport_Type (Ctx, Struct.Transport_Type);
      Set_Size (Ctx, Struct.Size);
      Set_SPDM_Payload (Ctx, Struct.SPDM_Payload (Struct.SPDM_Payload'First .. Struct.SPDM_Payload'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Size) * 8) + 1) - 2));
   end To_Context;
