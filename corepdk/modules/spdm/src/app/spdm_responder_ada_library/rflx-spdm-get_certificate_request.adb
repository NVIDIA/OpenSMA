...
package body RFLX.SPDM.Get_Certificate_Request with
  SPARK_Mode
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM.Get_Certificate_Request.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM.Get_Certificate_Request.Valid_Predecessor (Ctx, Fld);
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.SPDM.Get_Certificate_Request.Has_Buffer (Ctx)
       and RFLX.SPDM.Get_Certificate_Request.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM.Get_Certificate_Request.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Length then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM.Get_Certificate_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Certificate_Request.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM.Get_Certificate_Request.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Get_Certificate_Request.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM.Get_Certificate_Request.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (case Fld is
                    ...
                    when F_Param_2 =>
                       (Predecessor (Ctx, F_Offset) = F_Param_2
                        and Valid_Next (Ctx, F_Offset)),
                    when F_Offset =>
                       Get_Offset (Ctx) = To_Actual (Val)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM.Get_Certificate_Request.Valid_Next (Ctx, Fld)
       and then Fld in F_Slot | F_Param_2 | F_Offset | F_Length
       and then RFLX.SPDM.Get_Certificate_Request.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Get_Certificate_Request.Valid_Size (Ctx, Fld, RFLX.SPDM.Get_Certificate_Request.Field_Size (Ctx, Fld))
       and then RFLX.SPDM.Get_Certificate_Request.Available_Space (Ctx, Fld) >= RFLX.SPDM.Get_Certificate_Request.Field_Size (Ctx, Fld)
       and then RFLX.SPDM.Get_Certificate_Request.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM.Get_Certificate_Request.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Offset (Ctx : in out Context; Val : RFLX.SPDM.Offset) is
   begin
      Set_Scalar (Ctx, F_Offset, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Offset;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.SPDM.Cert_Length) is
   begin
