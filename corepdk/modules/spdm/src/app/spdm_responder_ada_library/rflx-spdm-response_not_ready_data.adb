...
package body RFLX.SPDM.Response_Not_Ready_Data with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, [F_RDT_Exponent => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, [F_RDT_Exponent => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM.Response_Not_Ready_Data.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM.Response_Not_Ready_Data.Valid_Predecessor (Ctx, Fld);
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          when F_RDT_Exponent =>
             Invalid (Ctx.Cursors (F_Request_Code)),
          when F_Request_Code =>
             Invalid (Ctx.Cursors (F_Token)),
          when F_Token =>
             Invalid (Ctx.Cursors (F_RDTM)),
          when F_RDTM =>
             True));
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.SPDM.Response_Not_Ready_Data.Has_Buffer (Ctx)
       and RFLX.SPDM.Response_Not_Ready_Data.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM.Response_Not_Ready_Data.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_RDTM then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM.Response_Not_Ready_Data.Has_Buffer (Ctx)
       and then RFLX.SPDM.Response_Not_Ready_Data.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM.Response_Not_Ready_Data.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Response_Not_Ready_Data.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM.Response_Not_Ready_Data.Available_Space (Ctx, Fld)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM.Response_Not_Ready_Data.Valid_Next (Ctx, Fld)
       and then Fld in F_RDT_Exponent | F_Request_Code | F_Token | F_RDTM
       and then RFLX.SPDM.Response_Not_Ready_Data.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Response_Not_Ready_Data.Valid_Size (Ctx, Fld, RFLX.SPDM.Response_Not_Ready_Data.Field_Size (Ctx, Fld))
       and then RFLX.SPDM.Response_Not_Ready_Data.Available_Space (Ctx, Fld) >= RFLX.SPDM.Response_Not_Ready_Data.Field_Size (Ctx, Fld)
       and then RFLX.SPDM.Response_Not_Ready_Data.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM.Response_Not_Ready_Data.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_RDT_Exponent (Ctx : in out Context; Val : RFLX.SPDM.RDT_Exponent) is
   begin
      Set_Scalar (Ctx, F_RDT_Exponent, RFLX.SPDM.To_Base_Integer (Val));
   end Set_RDT_Exponent;

   procedure Set_Request_Code (Ctx : in out Context; Val : RFLX.SPDM.Request_Code) is
   begin
      Set_Scalar (Ctx, F_Request_Code, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Request_Code;

   procedure Set_Token (Ctx : in out Context; Val : RFLX.SPDM.Token) is
   begin
      Set_Scalar (Ctx, F_Token, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Token;

   procedure Set_RDTM (Ctx : in out Context; Val : RFLX.SPDM.RDTM) is
   begin
      Set_Scalar (Ctx, F_RDTM, RFLX.SPDM.To_Base_Integer (Val));
   end Set_RDTM;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.RDT_Exponent := Get_RDT_Exponent (Ctx);
      Struct.Request_Code := Get_Request_Code (Ctx);
      Struct.Token := Get_Token (Ctx);
      Struct.RDTM := Get_RDTM (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_RDT_Exponent (Ctx, Struct.RDT_Exponent);
      Set_Request_Code (Ctx, Struct.Request_Code);
      Set_Token (Ctx, Struct.Token);
      Set_RDTM (Ctx, Struct.RDTM);
   end To_Context;
