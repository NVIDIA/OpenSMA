...
package body RFLX.SPDM.Ext_Alg with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, [F_Registry_ID => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, [F_Registry_ID => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM.Ext_Alg.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM.Ext_Alg.Valid_Predecessor (Ctx, Fld);
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          ...
          when F_Reserved =>
             Invalid (Ctx.Cursors (F_Algorithm_ID_16))
             and Invalid (Ctx.Cursors (F_Algorithm_ID_32)),
          when F_Algorithm_ID_16 | F_Algorithm_ID_32 =>
             True));
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.SPDM.Ext_Alg.Has_Buffer (Ctx)
       and RFLX.SPDM.Ext_Alg.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM.Ext_Alg.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if
                                  Fld = F_Algorithm_ID_16
                                  or Fld = F_Algorithm_ID_32
                                  or Fld = F_Reserved
                               then
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.SPDM.Ext_Alg.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM.Ext_Alg.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (case Fld is
                    ...
                    when F_Algorithm_ID_16 =>
                       Get_Algorithm_ID_16 (Ctx) = To_Actual (Val)
                       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)),
                    when F_Algorithm_ID_32 =>
                       Get_Algorithm_ID_32 (Ctx) = To_Actual (Val)
                       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM.Ext_Alg.Valid_Next (Ctx, Fld)
       and then Fld in F_Registry_ID | F_Reserved | F_Algorithm_ID_16 | F_Algorithm_ID_32
       and then RFLX.SPDM.Ext_Alg.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Ext_Alg.Valid_Size (Ctx, Fld, RFLX.SPDM.Ext_Alg.Field_Size (Ctx, Fld))
       and then RFLX.SPDM.Ext_Alg.Available_Space (Ctx, Fld) >= RFLX.SPDM.Ext_Alg.Field_Size (Ctx, Fld)
       and then RFLX.SPDM.Ext_Alg.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM.Ext_Alg.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Algorithm_ID_16 (Ctx : in out Context; Val : RFLX.SPDM.Algorithm_ID_16) is
   begin
      Set_Scalar (Ctx, F_Algorithm_ID_16, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Algorithm_ID_16;

   procedure Set_Algorithm_ID_32 (Ctx : in out Context; Val : RFLX.SPDM.Algorithm_ID_32) is
   begin
      Set_Scalar (Ctx, F_Algorithm_ID_32, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Algorithm_ID_32;
