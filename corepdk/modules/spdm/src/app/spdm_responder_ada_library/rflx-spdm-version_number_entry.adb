...
package body RFLX.SPDM.Version_Number_Entry with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, [F_Alpha => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, [F_Alpha => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
     ((case Fld is
          when F_Alpha =>
             (if Ctx.Cursors (F_Alpha).Value = 0 then F_Update_Version_Number else F_Initial),
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM.Version_Number_Entry.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM.Version_Number_Entry.Valid_Predecessor (Ctx, Fld);
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          when F_Alpha =>
             Invalid (Ctx.Cursors (F_Update_Version_Number)),
          when F_Update_Version_Number =>
             Invalid (Ctx.Cursors (F_Minor_Version)),
          when F_Minor_Version =>
             Invalid (Ctx.Cursors (F_Major_Version)),
          when F_Major_Version =>
             True));
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.SPDM.Version_Number_Entry.Has_Buffer (Ctx)
       and RFLX.SPDM.Version_Number_Entry.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM.Version_Number_Entry.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Major_Version then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.SPDM.Version_Number_Entry.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM.Version_Number_Entry.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (case Fld is
                    ...
                    when F_Update_Version_Number =>
                       Get_Update_Version_Number (Ctx) = To_Actual (Val)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM.Version_Number_Entry.Valid_Next (Ctx, Fld)
       and then Fld in F_Alpha | F_Update_Version_Number | F_Minor_Version | F_Major_Version
       and then RFLX.SPDM.Version_Number_Entry.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Version_Number_Entry.Valid_Size (Ctx, Fld, RFLX.SPDM.Version_Number_Entry.Field_Size (Ctx, Fld))
       and then RFLX.SPDM.Version_Number_Entry.Available_Space (Ctx, Fld) >= RFLX.SPDM.Version_Number_Entry.Field_Size (Ctx, Fld)
       and then RFLX.SPDM.Version_Number_Entry.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM.Version_Number_Entry.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Alpha (Ctx : in out Context; Val : RFLX.SPDM.Alpha) is
   begin
      Set_Scalar (Ctx, F_Alpha, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Alpha;

   procedure Set_Update_Version_Number (Ctx : in out Context; Val : RFLX.SPDM.Update_Version_Number) is
   begin
      Set_Scalar (Ctx, F_Update_Version_Number, RFLX.SPDM.To_Base_Integer (Val));
   end Set_Update_Version_Number;
   ...
   begin
      Struct.Alpha := Get_Alpha (Ctx);
      Struct.Update_Version_Number := Get_Update_Version_Number (Ctx);
      Struct.Minor_Version := Get_Minor_Version (Ctx);
      Struct.Major_Version := Get_Major_Version (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Alpha (Ctx, Struct.Alpha);
      Set_Update_Version_Number (Ctx, Struct.Update_Version_Number);
      Set_Minor_Version (Ctx, Struct.Minor_Version);
      Set_Major_Version (Ctx, Struct.Major_Version);
   end To_Context;
