...
package body RFLX.SPDM.Get_Version_Request with
  SPARK_Mode
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM.Get_Version_Request.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM.Get_Version_Request.Valid_Predecessor (Ctx, Fld);
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM.Get_Version_Request.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Param_2 then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM.Get_Version_Request.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Get_Version_Request.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM.Get_Version_Request.Available_Space (Ctx, Fld)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, Fld)
       and then Fld in F_Param_1 | F_Param_2
       and then RFLX.SPDM.Get_Version_Request.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Get_Version_Request.Valid_Size (Ctx, Fld, RFLX.SPDM.Get_Version_Request.Field_Size (Ctx, Fld))
       and then RFLX.SPDM.Get_Version_Request.Available_Space (Ctx, Fld) >= RFLX.SPDM.Get_Version_Request.Field_Size (Ctx, Fld)
       and then RFLX.SPDM.Get_Version_Request.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM.Get_Version_Request.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
