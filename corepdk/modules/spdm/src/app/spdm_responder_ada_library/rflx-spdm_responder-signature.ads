...
package RFLX.SPDM_Responder.Signature with
  SPARK_Mode,
  Annotate =>
...
is
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Take_Buffer (Ctx : in out Context; Buffer : out RFLX_Types.Bytes_Ptr) with
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and Buffer /= null
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Structural_Valid_Message (Ctx)
       and then RFLX.SPDM_Responder.Signature.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then Offset < RFLX.SPDM_Responder.Signature.Buffer_Length (Ctx)
       and then Pre (RFLX.SPDM_Responder.Signature.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Structural_Valid_Message (Ctx)
       and then Data'Length = RFLX.SPDM_Responder.Signature.Byte_Size (Ctx);
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM_Responder.Signature.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Condition (Ctx : Context; Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean with
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Valid_Predecessor (Ctx, Fld)
       and then RFLX.SPDM_Responder.Signature.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM_Responder.Signature.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length with
     Pre =>
       RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Data =>
   ...
   function Equal (Ctx : Context; Fld : Field; Data : RFLX_Types.Bytes) return Boolean with
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.SPDM.Signature_Length with
     Pre =>
       RFLX.SPDM_Responder.Signature.Valid (Ctx, RFLX.SPDM_Responder.Signature.F_Length);
   ...
   function Get_Data (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Structural_Valid (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, RFLX.SPDM_Responder.Signature.F_Data),
     Post =>
   ...
   procedure Get_Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Structural_Valid (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then Data'Length = RFLX_Types.To_Length (RFLX.SPDM_Responder.Signature.Field_Size (Ctx, RFLX.SPDM_Responder.Signature.F_Data)),
     Post =>
   ...
   procedure Generic_Get_Data (Ctx : Context) with
     Pre =>
       RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and RFLX.SPDM_Responder.Signature.Present (Ctx, RFLX.SPDM_Responder.Signature.F_Data);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.SPDM.Signature_Length) with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, RFLX.SPDM_Responder.Signature.F_Length)
       and then RFLX.SPDM.Valid_Signature_Length (RFLX.SPDM.To_Base_Integer (Val))
       and then RFLX.SPDM_Responder.Signature.Available_Space (Ctx, RFLX.SPDM_Responder.Signature.F_Length) >= RFLX.SPDM_Responder.Signature.Field_Size (Ctx, RFLX.SPDM_Responder.Signature.F_Length)
       and then RFLX.SPDM_Responder.Signature.Field_Condition (Ctx, RFLX.SPDM_Responder.Signature.F_Length, RFLX.SPDM.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       ...
       and (if
               Get_Length (Ctx) = 512
               or Get_Length (Ctx) = 384
               or Get_Length (Ctx) = 256
               or Get_Length (Ctx) = 132
               or Get_Length (Ctx) = 64
               or Get_Length (Ctx) = 96
            then
   ...
   procedure Initialize_Data (Ctx : in out Context) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Available_Space (Ctx, RFLX.SPDM_Responder.Signature.F_Data) >= RFLX.SPDM_Responder.Signature.Field_Size (Ctx, RFLX.SPDM_Responder.Signature.F_Data),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Available_Space (Ctx, RFLX.SPDM_Responder.Signature.F_Data) >= RFLX.SPDM_Responder.Signature.Field_Size (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Valid_Length (Ctx, RFLX.SPDM_Responder.Signature.F_Data, Data'Length)
       and then RFLX.SPDM_Responder.Signature.Available_Space (Ctx, RFLX.SPDM_Responder.Signature.F_Data) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.SPDM_Responder.Signature.Field_Condition (Ctx, RFLX.SPDM_Responder.Signature.F_Data, 0),
     Post =>
       Has_Buffer (Ctx)
       and Structural_Valid (Ctx, F_Data)
       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Data))
   ...
   procedure Generic_Set_Data (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Valid_Next (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Available_Space (Ctx, RFLX.SPDM_Responder.Signature.F_Data) >= RFLX.SPDM_Responder.Signature.Field_Size (Ctx, RFLX.SPDM_Responder.Signature.F_Data)
       and then RFLX.SPDM_Responder.Signature.Valid_Length (Ctx, RFLX.SPDM_Responder.Signature.F_Data, Length)
       and then RFLX_Types.To_Length (RFLX.SPDM_Responder.Signature.Available_Space (Ctx, RFLX.SPDM_Responder.Signature.F_Data)) >= Length
   ...
   type Structure is
      record
         ...
         Data : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 511);
      end record;
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Responder.Signature.Has_Buffer (Ctx)
       and then RFLX.SPDM_Responder.Signature.Valid_Structure (Struct)
       and then RFLX.SPDM_Responder.Signature.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
      ...
      and then ((if
                 ...
                 then
                    (Valid (Cursors (F_Length))
                     ...
                     and then (Cursors (F_Length).Value = 512
                               or Cursors (F_Length).Value = 384
                               or Cursors (F_Length).Value = 256
                               or Cursors (F_Length).Value = 132
                               or Cursors (F_Length).Value = 64
                               or Cursors (F_Length).Value = 96))))
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.SPDM_Responder.Signature.Field_First (Ctx, RFLX.SPDM_Responder.Signature.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     ((case Fld is
          when F_Length =>
             RFLX.SPDM.Valid_Signature_Length (Val),
          when F_Data =>
             True));
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean is
     ((case Ctx.Cursors (Fld).Predecessor is
          when F_Initial | F_Data | F_Final =>
             True,
          when F_Length =>
             Ctx.Cursors (F_Length).Value = 512
             or Ctx.Cursors (F_Length).Value = 384
             or Ctx.Cursors (F_Length).Value = 256
             or Ctx.Cursors (F_Length).Value = 132
             or Ctx.Cursors (F_Length).Value = 64
             or Ctx.Cursors (F_Length).Value = 96));
   ...
   function Get_Length (Ctx : Context) return RFLX.SPDM.Signature_Length is
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     ((RFLX_Types.Base_Integer (Struct.Length) = 512
       or RFLX_Types.Base_Integer (Struct.Length) = 384
       or RFLX_Types.Base_Integer (Struct.Length) = 256
       or RFLX_Types.Base_Integer (Struct.Length) = 132
       or RFLX_Types.Base_Integer (Struct.Length) = 64
       or RFLX_Types.Base_Integer (Struct.Length) = 96));
...
end RFLX.SPDM_Responder.Signature;
