...
package RFLX.SPDM.Get_Version_Request with
  SPARK_Mode,
  Annotate =>
...
is
   ...
   type Virtual_Field is (F_Initial, F_Param_1, F_Param_2, F_Final);

   subtype Field is Virtual_Field range F_Param_1 .. F_Param_2;
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Take_Buffer (Ctx : in out Context; Buffer : out RFLX_Types.Bytes_Ptr) with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and Buffer /= null
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Structural_Valid_Message (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then Offset < RFLX.SPDM.Get_Version_Request.Buffer_Length (Ctx)
       and then Pre (RFLX.SPDM.Get_Version_Request.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Structural_Valid_Message (Ctx)
       and then Data'Length = RFLX.SPDM.Get_Version_Request.Byte_Size (Ctx);
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Valid_Predecessor (Ctx, Fld)
       and then RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM.Get_Version_Request.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, Fld);
   ...
   function Get_Param_1 (Ctx : Context) return RFLX.SPDM.Reserved_8 with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Valid (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_1);

   function Get_Param_2 (Ctx : Context) return RFLX.SPDM.Reserved_8 with
     Pre =>
       RFLX.SPDM.Get_Version_Request.Valid (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_2);
   ...
   procedure Set_Param_1 (Ctx : in out Context; Val : RFLX.SPDM.Reserved_8) with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_1)
       and then RFLX.SPDM.Valid_Reserved_8 (RFLX.SPDM.To_Base_Integer (Val))
       and then RFLX.SPDM.Get_Version_Request.Available_Space (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_1) >= RFLX.SPDM.Get_Version_Request.Field_Size (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_1)
       and then RFLX.SPDM.Get_Version_Request.Field_Condition (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_1),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Param_2 (Ctx : in out Context; Val : RFLX.SPDM.Reserved_8) with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Valid_Next (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_2)
       and then RFLX.SPDM.Valid_Reserved_8 (RFLX.SPDM.To_Base_Integer (Val))
       and then RFLX.SPDM.Get_Version_Request.Available_Space (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_2) >= RFLX.SPDM.Get_Version_Request.Field_Size (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_2)
       and then RFLX.SPDM.Get_Version_Request.Field_Condition (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_2),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Param_2)
       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Param_2))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM.Get_Version_Request.Has_Buffer (Ctx)
       and then RFLX.SPDM.Get_Version_Request.Valid_Structure (Struct)
       and then RFLX.SPDM.Get_Version_Request.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.SPDM.Get_Version_Request.Field_First (Ctx, RFLX.SPDM.Get_Version_Request.F_Param_1) rem RFLX_Types.Byte'Size = 1
...
end RFLX.SPDM.Get_Version_Request;
