...
package body RFLX.SPDM.Slot_Mask with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, [F_Slot_7_Present => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, [F_Slot_7_Present => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM.Slot_Mask.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM.Slot_Mask.Valid_Predecessor (Ctx, Fld);
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          ...
          when F_Slot_1_Present =>
             Invalid (Ctx.Cursors (F_Slot_0_Present)),
          when F_Slot_0_Present =>
             True));
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.SPDM.Slot_Mask.Has_Buffer (Ctx)
       and RFLX.SPDM.Slot_Mask.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM.Slot_Mask.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Slot_0_Present then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM.Slot_Mask.Has_Buffer (Ctx)
       and then RFLX.SPDM.Slot_Mask.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM.Slot_Mask.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Slot_Mask.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM.Slot_Mask.Available_Space (Ctx, Fld)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM.Slot_Mask.Valid_Next (Ctx, Fld)
       and then Fld in F_Slot_7_Present | F_Slot_6_Present | F_Slot_5_Present | F_Slot_4_Present | F_Slot_3_Present | F_Slot_2_Present | F_Slot_1_Present | F_Slot_0_Present
       and then RFLX.SPDM.Slot_Mask.Valid_Value (Fld, Val)
       and then RFLX.SPDM.Slot_Mask.Valid_Size (Ctx, Fld, RFLX.SPDM.Slot_Mask.Field_Size (Ctx, Fld))
       and then RFLX.SPDM.Slot_Mask.Available_Space (Ctx, Fld) >= RFLX.SPDM.Slot_Mask.Field_Size (Ctx, Fld)
       and then RFLX.SPDM.Slot_Mask.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM.Slot_Mask.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
