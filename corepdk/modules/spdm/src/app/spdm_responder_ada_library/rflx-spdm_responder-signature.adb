...
package body RFLX.SPDM_Responder.Signature with
  SPARK_Mode
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM_Responder.Signature.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM_Responder.Signature.Valid_Predecessor (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.SPDM_Responder.Signature.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM_Responder.Signature.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.SPDM_Responder.Signature.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Responder.Signature.Valid_Size (Ctx, Fld, RFLX.SPDM_Responder.Signature.Field_Size (Ctx, Fld))
       and then RFLX.SPDM_Responder.Signature.Available_Space (Ctx, Fld) >= RFLX.SPDM_Responder.Signature.Field_Size (Ctx, Fld)
       and then RFLX.SPDM_Responder.Signature.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM_Responder.Signature.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.SPDM.Signature_Length) is
   begin
   ...
   procedure Initialize_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM_Responder.Signature.Field_First (Ctx, RFLX.SPDM_Responder.Signature.F_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
