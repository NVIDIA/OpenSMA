...
package body RFLX.SPDM_Requester.Request with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, [F_Size => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, [F_Size => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)]);
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM_Requester.Request.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM_Requester.Request.Valid_Predecessor (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.SPDM_Requester.Request.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM_Requester.Request.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Requester.Request.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM_Requester.Request.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.SPDM_Requester.Request.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, Fld)
       and then Fld in F_Size
       and then RFLX.SPDM_Requester.Request.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Requester.Request.Valid_Size (Ctx, Fld, RFLX.SPDM_Requester.Request.Field_Size (Ctx, Fld))
       and then RFLX.SPDM_Requester.Request.Available_Space (Ctx, Fld) >= RFLX.SPDM_Requester.Request.Field_Size (Ctx, Fld)
       and then RFLX.SPDM_Requester.Request.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.SPDM_Requester.Request.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Size (Ctx : in out Context; Val : RFLX.SPDM_Requester.Size) is
   begin
      Set_Scalar (Ctx, F_Size, RFLX.SPDM_Requester.To_Base_Integer (Val));
   end Set_Size;
   ...
   procedure Initialize_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Field_First (Ctx, RFLX.SPDM_Requester.Request.F_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
   ...
   begin
      ...
      Set_Data (Ctx, Struct.Data (Struct.Data'First .. Struct.Data'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Size) * 8) + 1) - 2));
   end To_Context;
