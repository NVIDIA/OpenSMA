...
is
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.SPDM.Nonce.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Take_Buffer (Ctx : in out Context; Buffer : out RFLX_Types.Bytes_Ptr) with
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and Buffer /= null
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then RFLX.SPDM.Nonce.Structural_Valid_Message (Ctx)
       and then RFLX.SPDM.Nonce.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then Offset < RFLX.SPDM.Nonce.Buffer_Length (Ctx)
       and then Pre (RFLX.SPDM.Nonce.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then RFLX.SPDM.Nonce.Structural_Valid_Message (Ctx)
       and then Data'Length = RFLX.SPDM.Nonce.Byte_Size (Ctx);
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM.Nonce.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then RFLX.SPDM.Nonce.Valid_Predecessor (Ctx, Fld)
       and then RFLX.SPDM.Nonce.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM.Nonce.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length with
     Pre =>
       RFLX.SPDM.Nonce.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Data =>
   ...
   function Equal (Ctx : Context; Fld : Field; Data : RFLX_Types.Bytes) return Boolean with
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and RFLX.SPDM.Nonce.Valid_Next (Ctx, Fld);
   ...
   function Get_Data (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then RFLX.SPDM.Nonce.Structural_Valid (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Valid_Next (Ctx, RFLX.SPDM.Nonce.F_Data),
     Post =>
   ...
   procedure Get_Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.SPDM.Nonce.Structural_Valid (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Valid_Next (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then Data'Length = RFLX_Types.To_Length (RFLX.SPDM.Nonce.Field_Size (Ctx, RFLX.SPDM.Nonce.F_Data)),
     Post =>
   ...
   procedure Generic_Get_Data (Ctx : Context) with
     Pre =>
       RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and RFLX.SPDM.Nonce.Present (Ctx, RFLX.SPDM.Nonce.F_Data);
   ...
   procedure Initialize_Data (Ctx : in out Context) with
     Pre =>
       ...
       and then RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then RFLX.SPDM.Nonce.Valid_Next (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Available_Space (Ctx, RFLX.SPDM.Nonce.F_Data) >= RFLX.SPDM.Nonce.Field_Size (Ctx, RFLX.SPDM.Nonce.F_Data),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.SPDM.Nonce.Valid_Next (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Available_Space (Ctx, RFLX.SPDM.Nonce.F_Data) >= RFLX.SPDM.Nonce.Field_Size (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Valid_Length (Ctx, RFLX.SPDM.Nonce.F_Data, Data'Length)
       and then RFLX.SPDM.Nonce.Available_Space (Ctx, RFLX.SPDM.Nonce.F_Data) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.SPDM.Nonce.Field_Condition (Ctx, RFLX.SPDM.Nonce.F_Data),
     Post =>
       Has_Buffer (Ctx)
       and Structural_Valid (Ctx, F_Data)
       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Data))
   ...
   procedure Generic_Set_Data (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM.Nonce.Valid_Next (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Available_Space (Ctx, RFLX.SPDM.Nonce.F_Data) >= RFLX.SPDM.Nonce.Field_Size (Ctx, RFLX.SPDM.Nonce.F_Data)
       and then RFLX.SPDM.Nonce.Valid_Length (Ctx, RFLX.SPDM.Nonce.F_Data, Length)
       and then RFLX_Types.To_Length (RFLX.SPDM.Nonce.Available_Space (Ctx, RFLX.SPDM.Nonce.F_Data)) >= Length
   ...
   type Structure is
      record
         Data : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 31);
      end record;
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM.Nonce.Has_Buffer (Ctx)
       and then RFLX.SPDM.Nonce.Valid_Structure (Struct)
       and then RFLX.SPDM.Nonce.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if Buffer /= null then Buffer'First = Buffer_First and Buffer'Last = Buffer_Last)
      ...
      and then (if
                ...
                then
                   Cursors (F_Data).Last - Cursors (F_Data).First + 1 = 256
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.SPDM.Nonce.Field_First (Ctx, RFLX.SPDM.Nonce.F_Data) rem RFLX_Types.Byte'Size = 1
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Unused_Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= 256);
