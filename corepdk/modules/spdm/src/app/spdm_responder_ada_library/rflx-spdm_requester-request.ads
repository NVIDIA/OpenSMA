...
package RFLX.SPDM_Requester.Request with
  SPARK_Mode,
  Annotate =>
...
is
   ...
   type Virtual_Field is (F_Initial, F_Size, F_Data, F_Final);

   subtype Field is Virtual_Field range F_Size .. F_Data;
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.SPDM_Requester.Request.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Take_Buffer (Ctx : in out Context; Buffer : out RFLX_Types.Bytes_Ptr) with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and Buffer /= null
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Structural_Valid_Message (Ctx)
       and then RFLX.SPDM_Requester.Request.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then Offset < RFLX.SPDM_Requester.Request.Buffer_Length (Ctx)
       and then Pre (RFLX.SPDM_Requester.Request.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Structural_Valid_Message (Ctx)
       and then Data'Length = RFLX.SPDM_Requester.Request.Byte_Size (Ctx);
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM_Requester.Request.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Valid_Predecessor (Ctx, Fld)
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, Fld)
       and then RFLX.SPDM_Requester.Request.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length with
     Pre =>
       RFLX.SPDM_Requester.Request.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Data =>
   ...
   function Equal (Ctx : Context; Fld : Field; Data : RFLX_Types.Bytes) return Boolean with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and RFLX.SPDM_Requester.Request.Valid_Next (Ctx, Fld);
   ...
   function Get_Size (Ctx : Context) return RFLX.SPDM_Requester.Size with
     Pre =>
       RFLX.SPDM_Requester.Request.Valid (Ctx, RFLX.SPDM_Requester.Request.F_Size);
   ...
   function Get_Data (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Structural_Valid (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, RFLX.SPDM_Requester.Request.F_Data),
     Post =>
   ...
   procedure Get_Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Structural_Valid (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then Data'Length = RFLX_Types.To_Length (RFLX.SPDM_Requester.Request.Field_Size (Ctx, RFLX.SPDM_Requester.Request.F_Data)),
     Post =>
   ...
   procedure Generic_Get_Data (Ctx : Context) with
     Pre =>
       RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and RFLX.SPDM_Requester.Request.Present (Ctx, RFLX.SPDM_Requester.Request.F_Data);
   ...
   procedure Set_Size (Ctx : in out Context; Val : RFLX.SPDM_Requester.Size) with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, RFLX.SPDM_Requester.Request.F_Size)
       and then RFLX.SPDM_Requester.Valid_Size (RFLX.SPDM_Requester.To_Base_Integer (Val))
       and then RFLX.SPDM_Requester.Request.Available_Space (Ctx, RFLX.SPDM_Requester.Request.F_Size) >= RFLX.SPDM_Requester.Request.Field_Size (Ctx, RFLX.SPDM_Requester.Request.F_Size)
       and then RFLX.SPDM_Requester.Request.Field_Condition (Ctx, RFLX.SPDM_Requester.Request.F_Size),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Size)
       and Get_Size (Ctx) = Val
       and Invalid (Ctx, F_Data)
       and (Predecessor (Ctx, F_Data) = F_Size
   ...
   procedure Set_Data_Empty (Ctx : in out Context) with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Available_Space (Ctx, RFLX.SPDM_Requester.Request.F_Data) >= RFLX.SPDM_Requester.Request.Field_Size (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Field_Condition (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Field_Size (Ctx, RFLX.SPDM_Requester.Request.F_Data) = 0,
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Available_Space (Ctx, RFLX.SPDM_Requester.Request.F_Data) >= RFLX.SPDM_Requester.Request.Field_Size (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Valid_Length (Ctx, RFLX.SPDM_Requester.Request.F_Data, Data'Length)
       and then RFLX.SPDM_Requester.Request.Available_Space (Ctx, RFLX.SPDM_Requester.Request.F_Data) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.SPDM_Requester.Request.Field_Condition (Ctx, RFLX.SPDM_Requester.Request.F_Data),
     Post =>
       Has_Buffer (Ctx)
       and Structural_Valid (Ctx, F_Data)
   ...
   procedure Generic_Set_Data (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM_Requester.Request.Valid_Next (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Available_Space (Ctx, RFLX.SPDM_Requester.Request.F_Data) >= RFLX.SPDM_Requester.Request.Field_Size (Ctx, RFLX.SPDM_Requester.Request.F_Data)
       and then RFLX.SPDM_Requester.Request.Valid_Length (Ctx, RFLX.SPDM_Requester.Request.F_Data, Length)
       and then RFLX_Types.To_Length (RFLX.SPDM_Requester.Request.Available_Space (Ctx, RFLX.SPDM_Requester.Request.F_Data)) >= Length
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.SPDM_Requester.Request.Has_Buffer (Ctx)
       and then RFLX.SPDM_Requester.Request.Valid_Structure (Struct)
       and then RFLX.SPDM_Requester.Request.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
      ...
      and then ((if
                 ...
                 then
                    (Valid (Cursors (F_Size))
                     and then Cursors (F_Data).Predecessor = F_Size)))
      and then ((if Invalid (Cursors (F_Size)) then Invalid (Cursors (F_Data))))
      and then (if
                   Structural_Valid (Cursors (F_Size))
                then
                   Cursors (F_Size).Last - Cursors (F_Size).First + 1 = 16
                   and then Cursors (F_Size).Predecessor = F_Initial
                   and then Cursors (F_Size).First = First
                   and then (if
                                Structural_Valid (Cursors (F_Data))
                             then
                                Cursors (F_Data).Last - Cursors (F_Data).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Size).Value) * 8
                                and then Cursors (F_Data).Predecessor = F_Size
                                and then Cursors (F_Data).First = Cursors (F_Size).Last + 1)));
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Size)
      and then RFLX.SPDM_Requester.Request.Field_First (Ctx, RFLX.SPDM_Requester.Request.F_Size) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Size) = Ctx.Last - Ctx.First + 1
      and then (for all F in Field =>
                   Invalid (Ctx, F)));
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     ((case Fld is
          when F_Size =>
             RFLX.SPDM_Requester.Valid_Size (Val),
          when F_Data =>
             True));
   ...
   function Field_First (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Index is
     ((if Fld = F_Size then Ctx.First else Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1));
   ...
   function Get_Size (Ctx : Context) return RFLX.SPDM_Requester.Size is
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.Base_Integer (Struct.Size) * 8 + 16);
...
end RFLX.SPDM_Requester.Request;
