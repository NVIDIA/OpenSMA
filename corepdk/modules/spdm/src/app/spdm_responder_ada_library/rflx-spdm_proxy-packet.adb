...
package body RFLX.SPDM_Proxy.Packet with
  SPARK_Mode
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.SPDM_Proxy.Packet.Structural_Valid (Ctx, Fld)
       and RFLX.SPDM_Proxy.Packet.Valid_Predecessor (Ctx, Fld);
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.SPDM_Proxy.Packet.Valid_Value (Fld, Val)
       and then RFLX.SPDM_Proxy.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.SPDM_Proxy.Packet.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.SPDM_Proxy.Packet.Composite_Field (Fld)
                 then
   ...
   procedure Initialize_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.SPDM_Proxy.Packet.Field_First (Ctx, RFLX.SPDM_Proxy.Packet.F_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
