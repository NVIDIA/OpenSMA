#include "crypto/ec/pdk-spdm-app-res-crypto-ec.h"

namespace pdk::spdm::app::res::crypto::ec {
...
{
    if constexpr (config::ECDHE_SUPPORT || config::ECDSA_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ec_new_by_nid(nid);
    }
    return nullptr;
}
...
{
    if constexpr (config::ECDHE_SUPPORT || config::ECDSA_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ec_free(ec_context);
    }
}
 ...
 *
 **/
bool libspdm_ec_set_priv_key(void*          ec_context,
                             const uint8_t* private_key,
                             size_t         private_key_size)
{
    if constexpr ((config::ECDHE_SUPPORT || config::ECDSA_SUPPORT) && config::FIPS_MODE) {
        return pdk::spdm::platforms::res::crypto::ec::ec_set_priv_key(
            ec_context, private_key, private_key_size);
    }
    return false;
}
 ...
 **/
bool libspdm_ec_set_pub_key(void* ec_context, const uint8_t* public_key, size_t public_key_size)
{
    if constexpr ((config::ECDHE_SUPPORT || config::ECDSA_SUPPORT) && config::FIPS_MODE) {
        return pdk::spdm::platforms::res::crypto::ec::ec_set_pub_key(
            ec_context, public_key, public_key_size);
    }
    return false;
}
...
{
    if constexpr (config::ECDHE_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ec_generate_key(
            ec_context, public_key, public_key_size);
    }
    return false;
}
...
{
    if constexpr (config::ECDHE_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ec_compute_key(
            ec_context, peer_public, peer_public_size, key, key_size);
    }
    return false;
}
...
bool libspdm_ec_get_public_key_from_der(const uint8_t* der_data,
                                        ...
                                        void**         ec_context)
{
    if constexpr (config::ECDSA_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ec_get_public_key_from_der(
            der_data, der_size, ec_context);
    }
    return false;
}
...
{
    if constexpr (config::ECDSA_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ecdsa_sign(
            ec_context, hash_nid, message_hash, hash_size, signature, sig_size);
    }
    return false;
}

/**
 * Carries out the EC-DSA signature with caller input random function. This API can be used for
 *FIPS test.
 *
 * @param[in]       ec_context    Pointer to EC context for signature generation.
 * @param[in]       hash_nid      hash NID
 ...
 * @retval  false  sig_size is too small.
 **/
bool libspdm_ecdsa_sign_ex(void*          ec_context,
                           size_t         hash_nid,
                           const uint8_t* message_hash,
                           size_t         hash_size,
                           uint8_t*       signature,
...
{
    if constexpr (config::ECDSA_SUPPORT && config::FIPS_MODE) {
        return pdk::spdm::platforms::res::crypto::ec::ecdsa_sign_ex(
            ec_context, hash_nid, message_hash, hash_size, signature, sig_size, random_func);
    }
    return false;
}
...
{
    if constexpr (config::ECDSA_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::ec::ecdsa_verify(
            ec_context, hash_nid, message_hash, hash_size, signature, sig_size);
    }
    return false;
}
...
// ====================================================================================
// Additional utility functions for algorithm support
// ====================================================================================

/**
 * Get the curve NID from the curve size for ECDSA/ECDHE operations
 */
size_t get_ec_nid_from_curve_size(size_t curve_size)
{
    switch (curve_size) {
        case constants::P256_PRIVATE_KEY_SIZE:  // P-256
            return constants::NID_X9_62_PRIME256V1;
        case constants::P384_PRIVATE_KEY_SIZE:  // P-384
            return constants::NID_SECP384R1;
        case constants::P521_PRIVATE_KEY_SIZE:  // P-521
            return constants::NID_SECP521R1;
        default: return 0;  // Invalid/unsupported curve
    }
}

/**
 * Get the hash NID from the hash size
 */
size_t get_hash_nid_from_size(size_t hash_size)
{
    switch (hash_size) {
        case constants::SHA256_HASH_SIZE: return constants::NID_SHA256;
        case constants::SHA384_HASH_SIZE: return constants::NID_SHA384;
        case constants::SHA512_HASH_SIZE: return constants::NID_SHA512;
        default                         : return 0;
    }
}
