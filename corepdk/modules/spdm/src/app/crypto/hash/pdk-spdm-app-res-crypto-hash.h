...
constexpr size_t SM3_256_DIGEST_SIZE = 32;
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SHA-256 context should be already correctly initialized by libspdm_sha256_init(),
 *and must not have been finalized by libspdm_sha256_final(). Behavior with invalid context
 *is undefined.
 *
 * If sha256_context is NULL, then return false.
 *
 ...
 *libspdm_sha256_init(), and must not have been finalized by libspdm_sha256_final().
 *Behavior with invalid SHA-256 context is undefined.
 *
 * If sha256_context is NULL, then return false.
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SHA-384 context should be already correctly initialized by libspdm_sha384_init(),
 *and must not have been finalized by libspdm_sha384_final(). Behavior with invalid context
 *is undefined.
 *
 * If sha384_context is NULL, then return false.
 *
 ...
 *libspdm_sha384_init(), and must not have been finalized by libspdm_sha384_final().
 *Behavior with invalid SHA-384 context is undefined.
 *
 * If sha384_context is NULL, then return false.
 * If hash_value is NULL, then return false.
 *
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SHA-512 context should be already correctly initialized by libspdm_sha512_init(),
 *and must not have been finalized by libspdm_sha512_final(). Behavior with invalid context
 *is undefined.
 *
 * If sha512_context is NULL, then return false.
 *
 ...
 *libspdm_sha512_init(), and must not have been finalized by libspdm_sha512_final().
 *Behavior with invalid SHA-512 context is undefined.
 *
 * If sha512_context is NULL, then return false.
 * If hash_value is NULL, then return false.
 *
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SHA3-256 context should be already correctly initialized by
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SHA3-384 context should be already correctly initialized by
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SHA3-512 context should be already correctly initialized by
 ...
 * It can be called multiple times to compute the digest of long or discontinuous data
 *streams. SM3 context should be already correctly initialized by sm3_init(), and should not
 *be finalized by sm3_final(). Behavior with invalid context is undefined.
 *
 * If sm3_context is NULL, then return false.
 *
 ...
 * be used again. SM3 context should be already correctly initialized by sm3_init(), and
 *should not be finalized by sm3_final(). Behavior with invalid SM3 context is undefined.
 *
 * If sm3_context is NULL, then return false.
 * If hash_value is NULL, then return false.
 *
