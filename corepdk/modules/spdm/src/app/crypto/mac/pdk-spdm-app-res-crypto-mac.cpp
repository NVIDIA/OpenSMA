#include "crypto/mac/pdk-spdm-app-res-crypto-mac.h"

namespace pdk::spdm::app::res::crypto::mac {
...
void* libspdm_hmac_sha256_new(void)
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha256_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sha256_free(hmac_sha256_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha256_set_key(
            hmac_sha256_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha256_duplicate(
            hmac_sha256_ctx, new_hmac_sha256_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha256_update(
            hmac_sha256_ctx, data, data_size);
    }
    return false;
}
 ...
 * be used again. HMAC-SHA256 context should be initialized by libspdm_hmac_sha256_new(), and
 *should not be finalized by libspdm_hmac_sha256_final(). Behavior with invalid HMAC-SHA256
...
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha256_final(hmac_sha256_ctx,
                                                                         hmac_value);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha256_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
...
void* libspdm_hmac_sha384_new(void)
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha384_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sha384_free(hmac_sha384_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha384_set_key(
            hmac_sha384_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha384_duplicate(
            hmac_sha384_ctx, new_hmac_sha384_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha384_update(
            hmac_sha384_ctx, data, data_size);
    }
    return false;
}
 ...
 * be used again. HMAC-SHA384 context should be initialized by libspdm_hmac_sha384_new(), and
 *should not be finalized by libspdm_hmac_sha384_final(). Behavior with invalid HMAC-SHA384
 *context is undefined.
 *
 * If hmac_sha384_ctx is NULL, then return false.
 * If hmac_value is NULL, then return false.
...
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha384_final(hmac_sha384_ctx,
                                                                         hmac_value);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha384_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
...
void* libspdm_hmac_sha512_new(void)
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha512_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sha512_free(hmac_sha512_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha512_set_key(
            hmac_sha512_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha512_duplicate(
            hmac_sha512_ctx, new_hmac_sha512_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha512_update(
            hmac_sha512_ctx, data, data_size);
    }
    return false;
}
 ...
 * be used again. HMAC-SHA512 context should be initialized by libspdm_hmac_sha512_new(), and
 *should not be finalized by libspdm_hmac_sha512_final(). Behavior with invalid HMAC-SHA512
 *context is undefined.
 *
 * If hmac_sha512_ctx is NULL, then return false.
 * If hmac_value is NULL, then return false.
...
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha512_final(hmac_sha512_ctx,
                                                                         hmac_value);
    }
    return false;
}
 ...
 *
 **/
bool libspdm_hmac_sha512_all(const void*    data,
                             size_t         data_size,
                             const uint8_t* key,
                             size_t         key_size,
                             uint8_t*       hmac_value)
{
    if constexpr (hash::config::SHA512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha512_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
...
void* libspdm_hmac_sha3_256_new(void)
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_free(hmac_sha3_256_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_set_key(
            hmac_sha3_256_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_duplicate(
            hmac_sha3_256_ctx, new_hmac_sha3_256_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_update(
            hmac_sha3_256_ctx, data, data_size);
    }
    return false;
}
 ...
 * be used again. HMAC-SHA3-256 context should be initialized by libspdm_hmac_sha3_256_new(),
 *and should not be finalized by libspdm_hmac_sha3_256_final(). Behavior with invalid
 *HMAC-SHA3-256 context is undefined.
 *
 * If hmac_sha3_256_ctx is NULL, then return false.
 * If hmac_value is NULL, then return false.
 *
 * @param[in, out]  hmac_sha3_256_ctx  Pointer to the HMAC-SHA3-256 context.
 * @param[out]      hmac_value          Pointer to a buffer that receives the HMAC-SHA3-256
...
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_final(hmac_sha3_256_ctx,
                                                                           hmac_value);
    }
    return false;
}
 ...
 * @retval false  This interface is not supported.
 **/
bool libspdm_hmac_sha3_256_all(const void*    data,
                               size_t         data_size,
                               const uint8_t* key,
                               size_t         key_size,
                               uint8_t*       hmac_value)
{
    if constexpr (hash::config::SHA3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_256_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
...
void* libspdm_hmac_sha3_384_new(void)
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_free(hmac_sha3_384_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_set_key(
            hmac_sha3_384_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_duplicate(
            hmac_sha3_384_ctx, new_hmac_sha3_384_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_update(
            hmac_sha3_384_ctx, data, data_size);
    }
    return false;
}
 ...
 * be used again. HMAC-SHA3-384 context should be initialized by libspdm_hmac_sha3_384_new(),
 *and should not be finalized by libspdm_hmac_sha3_384_final(). Behavior with invalid
 *HMAC-SHA3-384 context is undefined.
 *
 * If hmac_sha3_384_ctx is NULL, then return false.
 * If hmac_value is NULL, then return false.
 * If this interface is not supported, then return false.
 *
 * @param[in, out]  hmac_sha3_384_ctx  Pointer to the HMAC-SHA3-384 context.
 * @param[out]      hmac_value         Pointer to a buffer that receives the HMAC-SHA3-384
 *digest value (48 bytes).
 *
...
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_final(hmac_sha3_384_ctx,
                                                                           hmac_value);
    }
    return false;
}
 ...
 * @retval false  This interface is not supported.
 **/
bool libspdm_hmac_sha3_384_all(const void*    data,
                               size_t         data_size,
                               const uint8_t* key,
                               size_t         key_size,
                               uint8_t*       hmac_value)
{
    if constexpr (hash::config::SHA3_384_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_384_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
...
void* libspdm_hmac_sha3_512_new(void)
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_free(hmac_sha3_512_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_set_key(
            hmac_sha3_512_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_duplicate(
            hmac_sha3_512_ctx, new_hmac_sha3_512_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_update(
            hmac_sha3_512_ctx, data, data_size);
    }
    return false;
}
 ...
 * be used again. HMAC-SHA3-512 context should be initialized by libspdm_hmac_sha3_512_new(),
 *and should not be finalized by libspdm_hmac_sha3_512_final(). Behavior with invalid
 *HMAC-SHA3-512 context is undefined.
 *
 * If hmac_sha3_512_ctx is NULL, then return false.
 * If hmac_value is NULL, then return false.
 * If this interface is not supported, then return false.
 *
 * @param[in, out]  hmac_sha3_512_ctx  Pointer to the HMAC-SHA3-512 context.
 * @param[out]      hmac_value         Pointer to a buffer that receives the HMAC-SHA3-512
 *digest value (64 bytes).
 *
...
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_final(hmac_sha3_512_ctx,
                                                                           hmac_value);
    }
    return false;
}
 ...
 * @retval false  This interface is not supported.
 **/
bool libspdm_hmac_sha3_512_all(const void*    data,
                               size_t         data_size,
                               const uint8_t* key,
                               size_t         key_size,
                               uint8_t*       hmac_value)
{
    if constexpr (hash::config::SHA3_512_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sha3_512_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
...
void* libspdm_hmac_sm3_256_new(void)
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_new();
    }
    return nullptr;
}
...
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_free(hmac_sm3_256_ctx);
    }
    return;
}
...
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_set_key(
            hmac_sm3_256_ctx, key, key_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_duplicate(
            hmac_sm3_256_ctx, new_hmac_sm3_256_ctx);
    }
    return false;
}
...
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_update(
            hmac_sm3_256_ctx, data, data_size);
    }
    return false;
}
...
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_final(hmac_sm3_256_ctx,
                                                                          hmac_value);
    }
    return false;
}
 ...
 * @retval false  This interface is not supported.
 **/
bool libspdm_hmac_sm3_256_all(const void*    data,
                              size_t         data_size,
                              const uint8_t* key,
                              size_t         key_size,
                              uint8_t*       hmac_value)
{
    if constexpr (hash::config::SM3_256_SUPPORT) {
        return pdk::spdm::platforms::res::crypto::mac::hmac_sm3_256_all(
            data, data_size, key, key_size, hmac_value);
    }
    return false;
}
