...
package body Pdk.Pldm.Plat with
  SPARK_Mode => On
is
   ...
   procedure Enqueue
     (Q    : in out Queue;
      Item :        ARR_NvU8_IDX32)
   is
      Full_Item : Arr_Pldm_Tx_Msg_Buffer := (others => 0);
   begin
      if Is_Full (Q)
      then
         null;
      elsif Item'Length > NV_PLDM_MAX_TX_SIZE
      then
         null; -- Too long
      else
         for I in Item'Range
   ...
   begin
      ...
      then
         ...
         Item    := Q.Data (Q.Front).Data;
         Len     := Q.Data (Q.Front).Length;
   ...
   is
      ...
      function Uc_Stamp_Rec_To_U32 is new Ada.Unchecked_Conversion
        (Source => Stamp_Rec, Target => NvU32);
   begin
   ...
   begin
      ...
      To_Decimal_String
        (Value => Major, Decimal_String => Version_String (Offset .. (Offset + 3)));
      Offset                  := Offset + 4;
      Version_String (Offset) := Character'Pos ('.');
      Offset                  := Offset + 1;
      To_Decimal_String
        (Value => Minor, Decimal_String => Version_String (Offset .. (Offset + 1)));
      Offset                  := Offset + 2;
      Version_String (Offset) := Character'Pos ('.');
      Offset                  := Offset + 1;
      To_Decimal_String
        (Value => Patch, Decimal_String => Version_String (Offset .. (Offset + 3)));
      Offset                  := Offset + 4;
      Version_String (Offset) := Character'Pos ('.');
      Offset                  := Offset + 1;
      To_Decimal_String
        (Value => Build, Decimal_String => Version_String (Offset .. (Offset + 3)));
      Offset := Offset + 4;
   ...
   procedure Get_Active_Stamp (Stamp : out NvU32) is
   ...
   begin
      ...
      Populate_Stamp (Minor => Minor, Patch => Patch, Build => Build, Stamp => Stamp);
   ...
   begin
      ...
      Populate_Version_String
        (Major          => Major, Minor => Minor, Patch => Patch, Build => Build,
         Version_String => Version_String, Length => Length);

   end Get_Active_Version_String;

   procedure Get_Pending_Stamp (Stamp : out NvU32) is
