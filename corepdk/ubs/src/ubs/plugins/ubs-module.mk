...
# Projects should override these variables to generate to correct module git url
PROJECT_GIT_URL ?= $(shell git config --get remote.origin.url)
GIT_URL_REPLACE ?= ubs.git
GIT_URL_REPLACE_WITH ?= $(UBS_MODULE_TYPE)/$(UBS_MODULE_NAME)
MODULE_GIT_URL ?= $(subst $(GIT_URL_REPLACE),$(GIT_URL_REPLACE_WITH),$(PROJECT_GIT_URL))
...
module-add-run-generator: module-add-git
	$(ubs_module_conf_generator '$(UBS_PATH_PROJECTS)')
...
module-add-git: module-add-run-validator
	if git ls-remote $(MODULE_GIT_URL) > /dev/null 2>&1; then \
		git submodule add $(MODULE_GIT_URL) share/mod/$(UBS_MODULE_NAME) > /dev/null 2>&1; \
		echo module-add info: module added; \
		if cd share/mod/$(UBS_MODULE_NAME); git checkout v$(UBS_MODULE_VER_MAJOR).$(UBS_MODULE_VER_MINOR).$(UBS_MODULE_VER_BUILD) > /dev/null 2>&1; then \
			echo module-add info: module checkouted to the specify version; \
		else \
			cd ..; \
			echo module-add error: module version does not exists; \
			git submodule deinit -f share/mod/$(UBS_MODULE_NAME) > /dev/null 2>&1; \
			rm -rf .git/modules/share/mod/$(UBS_MODULE_NAME)> /dev/null 2>&1; \
			git rm -rf share/mod/$(UBS_MODULE_NAME)> /dev/null 2>&1; \
...
module-list-run-retriever:
	$(ubs_module_retriever '$(UBS_PATH_PROJECTS)')
