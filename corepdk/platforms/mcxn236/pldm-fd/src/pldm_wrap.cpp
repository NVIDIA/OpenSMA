...
template<>
void send_authenticate_firmware_result<nv::ipc::TaskId::Pldm>(
    AuthenticateFirmwareResponseParameter response)
{
    // send the auth result to pldm
    pldm::Request request{
        .type   = nv::pldm::RequestType::AuthFinish,
        .length = 1,
        .rsv1   = 0,
        .rsv2   = 0,
    };
...
void pldm_write(std::array<uint8_t, NV_PLDM_MAX_PAYLOAD_SIZE>& buffer,
                NvU32                                          offset,
                NvU32                                          size,
                NvU16&                                         err)
{
...
void pldm_update_pds(NvU8 status, NvU16& err)
{
...
void get_fw_info(NvU16& component_id,
                 NvU32& fw_size,
                 NvU32& fw_offset,
                 NvU32& ap_sku_id,
                 NvU8&  build_mode,
                 NvU8&  fw_state)
{
    component_id = sys::flash::config::McuComponentId;
...
{
    auto status = nv::spdm::crypto::authenticate_firmware(
...
void get_authentication_result(uint8_t result, uint8_t& verify_cc)
{
#ifdef NV_UNITTEST
...
void print_msg(char* msg, NvU32 data)
{
    nv::info(msg);
    nv::info(" 0x%x\n", data);
}
